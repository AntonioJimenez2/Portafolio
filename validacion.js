...
for (let i = 0; i < camposContacto.length; i++) {
  camposContacto[i].onkeyup = () => {
    if (
      camposContacto[0].value &&
      camposContacto[1].value &&
      camposContacto[2].value &&
      camposContacto[3].value
    ) {
...
function validar() {
  const nombre = document.querySelector(
    ".formcontato__input[name='nombre']"
  ).value;
  const email = document.querySelector(
    ".formcontato__input[name='email']"
  ).value;
  const asunto = document.querySelector(
    ".formcontato__input[name='asunto']"
  ).value;
